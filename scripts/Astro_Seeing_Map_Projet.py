...
def calculate_seeing(lat, lon):
    ...
    Cn2_integrated = np.nansum(
        Cn2, axis=0
    )  # yooo whaaaat heum,l'histoire d'additionner des nan maybe pas tres catholique
...
m = Basemap(
    projection="merc",
    llcrnrlat=lat_range.min(),
    urcrnrlat=lat_range.max(),
    llcrnrlon=lon_range.min(),
    urcrnrlon=lon_range.max(),
    resolution="i",
)
m.drawcoastlines()
m.drawcountries()
m.drawparallels(np.arange(-90.0, 91.0, 0.5), labels=[1, 0, 0, 0], fontsize=10)
m.drawmeridians(np.arange(-180.0, 181.0, 0.5), labels=[0, 0, 0, 1], fontsize=10)
